  ...
  const submitHandler = async (e) => {
    ...
    try {
      const resp = await axios.post(
        `https://graphical-pass-auth.onrender.com/validate-otp`,
        {
          email: userEmail,
          enteredOTP,
        }
      ...
      if (resp.data.success) {
        ...
        const response = await fetch(
          `https://graphical-pass-auth.onrender.com/delete/${email}`,
          {
            method: "DELETE",
            headers: {
        ...
        <form className="shadow-lg shadow-slate-500 rounded px-8 pt-6 pb-8 mb-4 w-[32vw]">
          ...
          <p className="text-[1.1rem] mx-5 mb-5">
            <span className="text-richblack-100">
              We have sent you a verification Email, check it and Enter the
              received OTP.
            </span>
            <br />
            <span className="text-blue-100 italic">
              Verify the Email ID with OTP and Reset your password.
            </span>
          </p>

          <div className="inp mb-4 flex flex-col">
            <label
              className="text-lg text-richblack-5 mb-1 mx-3"
              htmlFor="email"
            >
